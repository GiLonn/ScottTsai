...
parser.add_argument('--source', type=str, default="UCM")
parser.add_argument('--target', type=str, default="UCM")
...
target_train_loader = data_loader.load_training(args.root_path, args.target_train_dir, args.batch_size, kwargs)
...
def test(model):
    ...
    with torch.no_grad():
        ...
        print(args.test_dir, '  Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(
...
if __name__ == '__main__':
    model = models.AMRANNet(num_classes=2)
    print(args)
    # print(model)
    # print(args.source_dir, "->", args.test_dir)
    path = './record/' + args.source + '/' + args.target + '/'
