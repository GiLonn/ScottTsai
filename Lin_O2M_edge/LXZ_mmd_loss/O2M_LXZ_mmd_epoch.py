...
parser = argparse.ArgumentParser(description="LXZ_train")
...
parser.add_argument("--save_path", type=str, default="None", help='path to save models and log files')
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, CFG):
    ...
    for e in range(opt.epoch):
        ...
        # print(model.base_network.conv1.weight)
        # print("model.base_network.layer1[0].conv1.weight")
        # print(model.base_network.layer1[0].conv1.weight)
...
def load_data(src, tar, test,root_dir):
    ...
    source_loader ,source_index = data_loader.load_data(
        folder_src, opt.batch_size, True, CFG['kwargs'])
    target_train_loader ,target_index = data_loader.load_data(
        folder_tar, opt.batch_size, True, CFG['kwargs'])
    target_test_loader ,test_index= data_loader.load_data(
        folder_test, opt.batch_size, False, CFG['kwargs'])

    print(source_index)
    print(target_index)
    print(test_index)

    # source_index = {'negative': 1, 'positive': 0}
...
if __name__ == '__main__':
    ...
    source_loader, target_train_loader, target_test_loader = load_data(
        source_name, target_name, test_name,opt.data_path)

    # print(source_loader.class_to_idx,target_train_loader.class_to_idx,target_test_loader.class_to_idx)

    # model = models.Transfer_Net(
    #     CFG['n_class'], transfer_loss='mmd', base_net='resnet50').to(DEVICE)
    model = models.Transfer_Net(CFG['n_class'], transfer_loss='mmd', base_net='resnet18').to(DEVICE)
    ...
    # for param in model.base_network.layer1.parameters():
    #     param.requires_grad = False
    
    # for param in model.base_network.layer2[0].parameters():
    #     param.requires_grad = False
    
    # for param in model.base_network.layer2.parameters():
    #     param.requires_grad = False

    # for param in model.base_network.layer3[0].parameters():
    #     param.requires_grad = False

    # for param in model.base_network.layer3.parameters():
    #     param.requires_grad = False

    # for param in model.base_network.layer4[0].parameters():
    #     param.requires_grad = False

    # for param in model.base_network.layer4.parameters():
    #     param.requires_grad = False
    ...
    # KMM_weight = KMM_Lin.compute_kmm("E:\\AmigoChou\\Q2\\Training\\O2M_LXZ vs ZYM\\datasets\\apple\\sn_200_sp_200\\source\\train\\positive"
    #                                  ,"E:\\AmigoChou\\Q2\\Training\\O2M_LXZ vs ZYM\\datasets\\apple\\sn_200_sp_200target\\train\\positive")

    # print(CFG['data_path'])
    # print(os.path.join(CFG['data_path'],"\\source\\train\\positive"))
    # print("-------------------")
    # print(CFG['data_path'] + "\\source\\train\\positive")
    KMM_weight = KMM_Lin.compute_kmm(
        opt.kmm_source_path,opt.kmm_target_path)
    ...
    # torch.save(model.state_dict(), '25_50_50_params.pkl')
