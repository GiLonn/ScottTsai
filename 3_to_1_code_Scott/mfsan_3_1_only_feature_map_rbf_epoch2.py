...
def train(model, test_flag):
    ...
    for i in range(1, iteration + 1):
        ...
        optimizer.zero_grad()
        data_src1, data_tgt1, target_pred1,cls_loss, mmd_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mu3, mark=1)
        ...
        if i % (log_interval*2) == 0:
            ...
            # print("gamma: ", gamma)
            print('Train source1 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tcls_Loss: {:.6f}\tmmd_Loss: {:.6f}\t'.format(
        ...
        data_src2, data_tgt2, target_pred2, cls_loss, mmd_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mu3, mark=2)
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration))) - 1
        # gamma = opt.lambda_
        # print(gamma)
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
        ...
        if i % (log_interval*2) == 0:
            ...
            # print("learning rate：", LEARNING_RATE)
            # print("gamma: ", gamma)
            print('Train source2 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tcls_Loss: {:.6f}\tmmd_Loss: {:.6f}\t'.format(
                    i, 100. * i / iteration, loss.item(), cls_loss.item(), mmd_loss.item()))
        ...
        data_src3, data_tgt3, target_pred3,cls_loss, mmd_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mu3, mark=3)
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration))) - 1
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
...
def test(model, test_flag):
    ...
    with torch.no_grad():
        for data, target in target_test_loader:
            ...
            pred1, pred2, pred3 = model(data, test_flag,data_tgt = 0, label_src = 0, mu1 = 0, mu2 = 0, mu3 = 0, mark = 0)
            #pred1, pred2 = model(data, mark = 0)
            pred1 = torch.nn.functional.softmax(pred1, dim=1)
            pred2 = torch.nn.functional.softmax(pred2, dim=1)

            pred = ((KMM_weight_source1_prediction*pred1) + (KMM_weight_source2_prediction*pred2) + (KMM_weight_source3_prediction*pred3)) / 3
...
if __name__ == '__main__':
    ...
    path_source_train3 = './source_train_feature3.npy'
    path_source_train3_label = './source_train_feature_label3.npy'
