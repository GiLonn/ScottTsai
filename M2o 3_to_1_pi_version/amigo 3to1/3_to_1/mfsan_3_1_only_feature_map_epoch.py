...
parser = argparse.ArgumentParser(description="M2O_3_to_1_train")
    ...
    for i in range(1, iteration + 1):
        ...
        if i % log_interval == 0:
            ...
            if t_correct > correct:
                correct = t_correct
            print(source1_name, source2_name, source3_name, "to", target_test_name, "%s max correct:" % target_test_name, correct.item(), "\n")
        '''
...
def test(model):
    ...
    with torch.no_grad():
        ...
        print('\nsource1 accnum {}, source2 accnum {}，source3 accnum {}'.format(correct1.type(torch.float32), correct2.type(torch.float32), correct3.type(torch.float32)))

        test_total = 100*correct_test.type(torch.float32)/len(target_test_loader.dataset)

        logtest.append([test_total,tn, fp, fn, tp,correct1,correct2,correct3])
...
if __name__ == '__main__':
    ...
    KMM_weight_source1 = KMM_Lin.compute_kmm(path_train_source_1, path_train_target, path_train_source_1_label)
    KMM_weight_source2 = KMM_Lin.compute_kmm(path_train_source_2, path_train_target, path_train_source_2_label)    
    KMM_weight_source3 = KMM_Lin.compute_kmm(path_train_source_3, path_train_target, path_train_source_3_label)    
    ...
    KMM_weight_source1_pred = KMM_Lin_pred.compute_kmm(path_train_source_1, path_train_target, path_train_source_1_label)
    KMM_weight_source2_pred = KMM_Lin_pred.compute_kmm(path_train_source_2, path_train_target, path_train_source_2_label)    
    KMM_weight_source3_pred = KMM_Lin_pred.compute_kmm(path_train_source_3, path_train_target, path_train_source_3_label)    
