...
def train(model):
    ...
    scheduler=torch.optim.lr_scheduler.StepLR(optimizer,step_size=10,gamma=0.16)
    ...
    for i in range(1, iteration + 1):
        ...
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration))) - 1
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
        loss = KMM_weight_source3 * cls_loss + gamma * mmd_loss
        ...
        logtrain3.append([loss, cls_loss,KMM_weight_source3*cls_loss,mmd_loss,gamma*mmd_loss])
...
if __name__ == '__main__':
    ...
    KMM_weight_source3 = torch.from_numpy(KMM_weight_source3).float().to(DEVICE)
    print(KMM_weight_source3)

    train(model)

    # torch.save(model.state_dict(), 'M2O.pkl')
