...
def train(source_loader, target_train_loader, test_flag,  target_test_loader, KMM_weight, model, CFG, save_test_path, save_log_path, optimizer):
    ...
    for e in range(opt.epoch):
        ...
        for i in range(n_batch):
            ...
            #label_source = label_source[0]
            label_source = torch.squeeze(label_source, 1)
            #print(label_source.size())
            ...
            #source, target, label_source_pred, transfer_loss = model(data_source, data_target, label_source, mu, test_flag)
...
if __name__ == '__main__':
    ...
    kwargs = {'num_workers': 4, 'pin_memory': False, 'persistent_workers' : True} if opt.gpu_id == 0 or 1 else {}
