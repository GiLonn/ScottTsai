...
def train(model, test_flag):
    ...
    for i in range(1, iteration + 1):
        ...
        optimizer.zero_grad()
        data_src1, data_tgt1, target_pred1, cls_loss, transfer_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mark=1)
        gamma = 2 / (1 + math.exp(-10* (i) / (iteration) )) - 1
        # gamma = opt.lambda_
        # print(gamma)
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
        loss = KMM_weight_source1_cls * cls_loss + gamma * transfer_loss
        ...
        if i % (log_interval*2) == 0:
            ...
            # print("gamma: ", gamma)
            print('Train source1 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tcls_Loss: {:.6f}\ttransfer_Loss: {:.6f}\tl1_Loss: {:.6f}'.format(
        ...
        data_src2, data_tgt2, target_pred2, cls_loss, transfer_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mark=2)
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration))) - 1
        # gamma = opt.lambda_
        # print(gamma)
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
        #loss = KMM_weight_source2_cls * cls_loss + gamma * mmd_loss
        loss = KMM_weight_source2_cls * cls_loss + gamma * transfer_loss
        ...
        if i % (log_interval*2) == 0:
            ...
            # print("learning rate：", LEARNING_RATE)
            # print("gamma: ", gamma)
            print('Train source2 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tcls_Loss: {:.6f}\ttransfer_Loss: {:.6f}\tl1_Loss: {:.6f}'.format(
                    i, 100. * i / iteration, loss.item(), cls_loss.item(), transfer_loss.item(), l1_loss.item()))
        '''
        if i % log_interval == 0:
...
def test(model, test_flag):
    ...
    with torch.no_grad():
        for data, target in target_test_loader:
            ...
            pred1, pred2 = model(data, test_flag, data_tgt = 0, label_src = 0, mu1 = 0, mu2 = 0, mark = 0)
