...
class Transfer_Net(nn.Module):
    ...
    def forward(self, source, target, s_label, mu, test_flag, batch_num):
        ...
        transfer_loss, mu = self.adapt_loss(source_bottle, target_bottle, self.transfer_loss, s_label, t_label, mu, batch_num)
        ...
        return source, target,source_clf, t_label, transfer_loss, mu
    ...
    def adapt_loss(self, X, Y, adapt_loss, s_label, t_label, mu, batch_num):
        ...
        if adapt_loss == 'mmd':
            ...
            if batch_num == 0:
                mu = es.estimate_mu(X.detach().cpu().numpy(), s_label.detach().cpu().numpy(),Y.detach().cpu().numpy(), t_label.detach().cpu().numpy()) 
            transfer_loss = (1- mu) * cmmd_loss + mu * mmd_loss
            #print(mu)
        else:
            transfer_loss = 0
        return transfer_loss, mu
