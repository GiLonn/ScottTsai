...
parser.add_argument('--source3_dir', type=str, default="UCM")
parser.add_argument('--source4_dir', type=str, default="UCM")
...
parser.add_argument('--source3_class', type=str, default="None")
parser.add_argument('--source4_class', type=str, default="None")
...
parser.add_argument("--save_train_loss_name3", type=str, default='train_log2.csv', help='path to save models and log files')
parser.add_argument("--save_train_loss_name4", type=str, default='train_log2.csv', help='path to save models and log files')
...
def train(model, test_flag):
    ...
    for i in range(1, iteration + 1):
        ...
        optimizer.zero_grad()
        data_src1, data_tgt1, target_pred1,cls_loss, mmd_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mu3, mu4, mark=1)
        ...
        optimizer.step()
        mu1_last = mu1
        mu1 = es.estimate_mu(data_src1.detach().cpu().numpy(), source_label.detach().cpu().numpy(),
         data_tgt1.detach().cpu().numpy(), target_pred1.detach().cpu().numpy())
        print("mu1 :", mu1)
        ...
        data_src2, data_tgt2, target_pred2, cls_loss, mmd_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mu3, mu4,mark=2)
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration))) - 1
        # gamma = opt.lambda_
        # print(gamma)
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
        ...
        scheduler.step()
        mu2_last = mu2
        mu2 = es.estimate_mu(data_src2.detach().cpu().numpy(), source_label.detach().cpu().numpy(),
         data_tgt2.detach().cpu().numpy(), target_pred2.detach().cpu().numpy())
        print("mu2 :", mu2)
        ...
        data_src3, data_tgt3, target_pred3,cls_loss, mmd_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mu3,mu4,mark=3)
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration))) - 1
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
        ...
        scheduler.step()
        mu3_last = mu3
        mu3 = es.estimate_mu(data_src3.detach().cpu().numpy(), source_label.detach().cpu().numpy(),
         data_tgt3.detach().cpu().numpy(), target_pred3.detach().cpu().numpy())
        print("mu3 :", mu3)
        # print(scheduler.get_lr())
        ...
        if i % log_interval == 0:
            ...
            if t_correct > correct:
                correct = t_correct
            print(opt.source1_class, opt.source2_class, opt.source3_class, "to", opt.test_class, "%s max correct:" % opt.test_class, correct.item(), "\n")
        '''
        '''
        ...
        data_src4, data_tgt4, target_pred4,cls_loss, mmd_loss, l1_loss = model(source_data, test_flag, target_data, source_label, mu1, mu2, mu3, mu4,mark=4)
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration))) - 1
        # loss = cls_loss + gamma * (mmd_loss + l1_loss)
        ...
        scheduler.step()
        mu4_last = mu4
        mu4 = es.estimate_mu(data_src4.detach().cpu().numpy(), source_label.detach().cpu().numpy(),
         data_tgt4.detach().cpu().numpy(), target_pred4.detach().cpu().numpy())
        print("mu4 :", mu4)
        # print(scheduler.get_lr())
        #torch.cuda.synchronize()
        tEnd = time.time() #計時結束
        ...
        if i % log_interval == 0:
            ...
            if t_correct > correct:
                correct = t_correct
            print(opt.source1_class, opt.source2_class, opt.source3_class,opt.source4_class, "to", opt.test_class, "%s max correct:" % opt.test_class, correct.item(), "\n")
        '''
        '''
        logtrain3.append([loss, cls_loss,KMM_weight_source3_cls*cls_loss,mmd_loss,gamma*mmd_loss])
        np_log3 = np.array(logtrain3, dtype=float)
...
def test(model, test_flag):
    ...
    with torch.no_grad():
        for data, target in target_test_loader:
            ...
            pred1, pred2, pred3, pred4 = model(data, test_flag,data_tgt = 0, label_src = 0, mu1 = 0, mu2 = 0, mu3 = 0, mu4 = 0, mark = 0)
...
if __name__ == '__main__':
    ...
    path_source_train1 = os.path.join(opt.train_root_path, opt.source1_dir, 'source_train_feature.npy')
    path_source_train1_label = os.path.join(opt.train_root_path, opt.source1_dir, 'source_train_feature_label.npy')

    path_source_train2 = os.path.join(opt.train_root_path, opt.source2_dir, 'source_train_feature.npy')
    path_source_train2_label = os.path.join(opt.train_root_path, opt.source2_dir, 'source_train_feature_label.npy')

    path_source_train3 = os.path.join(opt.train_root_path, opt.source3_dir, 'source_train_feature.npy')
    path_source_train3_label = os.path.join(opt.train_root_path, opt.source3_dir, 'source_train_feature_label.npy')

    path_source_train4 = os.path.join(opt.train_root_path, opt.source3_dir, 'source_train_feature.npy')
    path_source_train4_label = os.path.join(opt.train_root_path, opt.source3_dir, 'source_train_feature_label.npy')
    ...
    sample_source_train_3 = torch.from_numpy(np.load(path_source_train3))
    sample_source_train3_label = torch.from_numpy(np.load(path_source_train3_label))
    sample_source_train_4 = torch.from_numpy(np.load(path_source_train4))
    sample_source_train4_label = torch.from_numpy(np.load(path_source_train4_label))
    ...
    source_3_dataset = Data.TensorDataset(sample_source_train_3, sample_source_train3_label)  
    source_4_dataset = Data.TensorDataset(sample_source_train_4, sample_source_train4_label)  
    ...
    KMM_weight_source3 = KMM_Lin.compute_kmm(path_source_train3, path_target_train, path_source_train3_label)
    KMM_weight_source4 = KMM_Lin.compute_kmm(path_source_train4, path_target_train, path_source_train4_label)
    ...
    KMM_weight_source3_pred = KMM_Lin_pred.compute_kmm(path_source_train3, path_target_train, path_source_train3_label)  
    KMM_weight_source4_pred = KMM_Lin_pred.compute_kmm(path_source_train4, path_target_train, path_source_train3_label)
    ...
    KMM_weight_source4_prediction = torch.from_numpy(KMM_weight_source3_pred*KMM_weight_source3_pred).float().to(DEVICE)
    print(KMM_weight_source3_prediction)
    ...
    save_log_train_3_path = './log_train_3/' + str(opt.com) + '/' + 'to' + '/' + opt.test_class + '/' +str(opt.lr)
    save_log_train_3_path = './log_train_4/' + str(opt.com) + '/' + 'to' + '/' + opt.test_class + '/' +str(opt.lr)
