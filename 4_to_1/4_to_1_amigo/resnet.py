...
# the dilation=1, groups=1 is defalut for nn.conv2d
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.avgpool = nn.AvgPool2d(7, stride=1) # 因上一層輸出為7*7，因此average pooling大小為7*7
...
class MFSAN(nn.Module):
    ...
    def forward(self, data_src, data_tgt = 0, label_src = 0, mark = 1):
        ...
        if self.training == True:
            data_src = self.sharedNet.layer4(data_src)
            data_tgt = self.sharedNet.layer4(data_tgt)
            ...
            if mark == 4:
                ...
                data_src = self.avgpool(data_src)
                data_src = data_src.view(data_src.size(0), -1)
                mmd_loss += mmd.mmd(data_src, data_tgt_son4)

                l1_loss = torch.mean( torch.abs(torch.nn.functional.softmax(data_tgt_son4, dim=1)
                                                - torch.nn.functional.softmax(data_tgt_son1, dim=1)) )
                l1_loss += torch.mean( torch.abs(torch.nn.functional.softmax(data_tgt_son4, dim=1)
                                                - torch.nn.functional.softmax(data_tgt_son2, dim=1)) )
                l1_loss += torch.mean( torch.abs(torch.nn.functional.softmax(data_tgt_son4, dim=1)
