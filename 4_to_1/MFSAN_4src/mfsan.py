...
source4_loader = data_loader.load_training(root_path, source4_name, batch_size, kwargs)
...
def train(model):
    ...
    for i in range(1, iteration + 1):
        ...
        logtrain1.append([loss, cls_loss,l1_loss,mmd_loss,gamma * (mmd_loss + l1_loss)])
        ...
        logtrain2.append([loss, cls_loss,l1_loss,mmd_loss,gamma * (mmd_loss + l1_loss)])
        ...
        logtrain3.append([loss, cls_loss,l1_loss,mmd_loss,gamma * (mmd_loss + l1_loss)])
        ...
        logtrain4.append([loss, cls_loss,l1_loss,mmd_loss,gamma * (mmd_loss + l1_loss)])
...
def test(model):
    ...
    with torch.no_grad():
        ...
        print(target_test_name, '\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(
            test_loss, correct.type(torch.float32), len(target_test_loader.dataset),
            100. * correct.type(torch.float32) / len(target_test_loader.dataset)))
