...
class MFSAN(nn.Module):
    ...
    def __init__(self, num_classes=2):
        ...
        multi_list3 = [conv_M2(512, 256), BasicConv2d(256, 256, kernel_size = 1, stride =1)]
        multi_list4 = [conv_M2(512, 256), BasicConv2d(256, 256, kernel_size = 1, stride =1)]


        self.sonnet1 = nn.Sequential(*multi_list)
        self.sonnet2 = nn.Sequential(*multi_list2)
        self.sonnet3 = nn.Sequential(*multi_list3)
        self.sonnet4 = nn.Sequential(*multi_list4)
        ...
        fc_list3 = [nn.Dropout(0.1), nn.Linear(256, num_classes)]
        fc_list4 = [nn.Dropout(0.1), nn.Linear(256, num_classes)]


        self.cls_fc_son1 = nn.Sequential(*fc_list1)
        self.cls_fc_son2 = nn.Sequential(*fc_list2)
        self.cls_fc_son3 = nn.Sequential(*fc_list3)
        self.cls_fc_son4 = nn.Sequential(*fc_list4)
        ...
        nn.init.xavier_normal_(self.cls_fc_son3[1].weight)
        nn.init.xavier_normal_(self.cls_fc_son4[1].weight)
    ...
    def forward(self, data_src, test_flag, data_tgt, label_src,mu1, mu2, mu3,mu4, mark = 1):
        mmd_loss = 0
        ...
        if self.training == True:
            ...
            data_tgt_label1 = Variable(pred_tgt_son1.data.max(1)[1])
            ...
            data_tgt_label2 = Variable(pred_tgt_son2.data.max(1)[1])
            ...
            data_tgt_label3 = Variable(pred_tgt_son3.data.max(1)[1])

            data_tgt_son4 = self.sonnet4(data_tgt)
            data_tgt_son4 = self.avgpool(data_tgt_son4)
            data_tgt_son4 = data_tgt_son4.view(data_tgt_son4.size(0), -1)
            pred_tgt_son4 = self.cls_fc_son3(data_tgt_son4)
            data_tgt_label4 = Variable(pred_tgt_son4.data.max(1)[1])
            ...
            if mark == 1:
                ...
                cmmd_loss = Variable(torch.tensor([0], device = 'cuda'))
                ...
                l1_loss =(torch.tensor([0], device = 'cuda'))
                ...
                return data_src, data_tgt_son1, data_tgt_label1, cls_loss, transfer_loss, l1_loss / 3
            ...
            if mark == 2:
                ...
                return data_src, data_tgt_son2, data_tgt_label2, cls_loss, transfer_loss, l1_loss / 3
            ...
            if mark == 3:
                ...
                data_src = self.avgpool(data_src)
                data_src = data_src.view(data_src.size(0), -1)
                mmd_loss += mmd.mmd_rbf_noaccelerate(data_src, data_tgt_son3)

                cmmd_loss = Variable(torch.tensor([0], device = 'cuda'))
                #cmmd_loss = cmmd_loss.cuda()
                cmmd_loss = mmd.cmmd(data_src, data_tgt_son3,label_src, data_tgt_label3)
                transfer_loss = (1- mu3) * cmmd_loss + mu3 * mmd_loss
                ...
                return data_src, data_tgt_son3,data_tgt_label3, cls_loss, transfer_loss, l1_loss / 3
            ...
            if mark==4:
                ...
                cmmd_loss = cmmd_loss.cuda()
                cmmd_loss = mmd.cmmd(data_src, data_tgt_son4,label_src, data_tgt_label4)
                transfer_loss = (1- mu4) * cmmd_loss + mu4 * mmd_loss
                ...
                return data_src, data_tgt_son4,data_tgt_label4, cls_loss, transfer_loss, l1_loss / 3
